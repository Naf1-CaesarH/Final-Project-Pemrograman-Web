...
<head>
  ...
  <title>Pengaturan Sistem — Portal Desa Digital</title>
...
<body class="bg-base font-sans text-t1 flex min-h-screen">
  ...
  <aside class="sidebar-fixed w-64 bg-slate-900 text-slate-300 flex flex-col justify-between sticky top-0 h-screen shadow-xl z-20">
    <div>
      ...
      <nav class="py-4 flex-1 overflow-y-auto scrollbar-hide">
        ...
        <a class="sb-item active !bg-blue-600/10 !text-blue-400 !border-r-4 !border-blue-500 px-5 py-2.5 flex items-center gap-3 cursor-pointer transition-colors" onclick="go('pengaturan')">
          ...
          <span class="font-semibold text-[13px]">Pengaturan Sistem</span>
        </a>
      </nav>
    </div>
  ...
  <div class="flex-1 flex flex-col min-h-screen relative overflow-hidden">
    ...
    <header class="h-[60px] bg-surface border-b border-border flex items-center justify-between px-6 sticky top-0 z-10">
      <h1 class="text-[15px] font-bold text-t1">Pengaturan Sistem</h1>
    </header>

    <main class="p-6 flex flex-col gap-6 flex-1 max-w-3xl mx-auto w-full">
      
      <section class="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col">
        <div class="p-5 border-b border-border">
          <h2 class="text-[15px] font-bold text-t1">Informasi Kelurahan</h2>
        </div>
        <div class="p-5 flex flex-col gap-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-[13px] font-semibold text-t1 mb-1.5">Nama Kelurahan</label>
              <input type="text" value="Kelurahan Sukamaju" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
            </div>
            <div>
              <label class="block text-[13px] font-semibold text-t1 mb-1.5">Kecamatan</label>
              <input type="text" value="Kecamatan Cibeunying Kidul" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
            </div>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-[13px] font-semibold text-t1 mb-1.5">Kota/Kabupaten</label>
              <input type="text" value="Kota Bandung" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
            </div>
            <div>
              <label class="block text-[13px] font-semibold text-t1 mb-1.5">Provinsi</label>
              <input type="text" value="Jawa Barat" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
            </div>
          </div>
          <div>
            <label class="block text-[13px] font-semibold text-t1 mb-1.5">Alamat Kantor Kelurahan</label>
            <input type="text" value="Jl. Sukamaju No. 12, Bandung" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
          </div>
          <div>
            <label class="block text-[13px] font-semibold text-t1 mb-1.5">Pesan selamat datang (portal)</label>
            <textarea class="w-full bg-base border border-border rounded-xl px-4 py-3 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all min-h-[90px] resize-y">Selamat datang di Portal Desa Digital Kelurahan Sukamaju. Layanan digital untuk warga yang lebih mudah dan cepat.</textarea>
          </div>
        </div>
        <div class="p-5 border-t border-border bg-slate-50 flex justify-end">
          <button type="button" class="px-5 py-2.5 rounded-xl bg-blue-600 text-white text-[13px] font-bold hover:bg-blue-700 transition-colors shadow-sm">
            Simpan perubahan
          </button>
        </div>
      </section>
      ...
      <section class="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col">
        <div class="p-5 border-b border-border">
          <h2 class="text-[15px] font-bold text-t1">Integrasi WhatsApp Gateway</h2>
          <p class="text-[12px] text-t2 mt-1">Digunakan untuk mengirim notifikasi dan pengumuman otomatis ke warga.</p>
        </div>
        <div class="p-5 flex flex-col gap-4">
          <div>
            <label class="block text-[13px] font-semibold text-t1 mb-1.5">URL API Gateway</label>
            <input type="url" value="https://wa-api.portaldesa.id/v1/send" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] font-mono text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-[13px] font-semibold text-t1 mb-1.5">API Key</label>
              <input type="password" value="••••••••••••••••••••" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] font-mono text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
            </div>
            <div>
              <label class="block text-[13px] font-semibold text-t1 mb-1.5">Nomor Pengirim</label>
              <input type="tel" value="+628111234567" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] font-mono text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
            </div>
          </div>
          <div>
            <label class="block text-[13px] font-semibold text-t1 mb-1.5">Nama Pengirim (ditampilkan di WA)</label>
            <input type="text" value="Kelurahan Sukamaju" class="w-full bg-base border border-border rounded-xl px-4 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
          </div>
        </div>
        <div class="p-5 border-t border-border bg-slate-50 flex justify-end gap-3">
          <button type="button" class="px-5 py-2.5 rounded-xl border border-border bg-white text-t1 text-[13px] font-bold hover:bg-base transition-colors shadow-sm">
            Test Kirim WA
          </button>
          <button type="button" class="px-5 py-2.5 rounded-xl bg-blue-600 text-white text-[13px] font-bold hover:bg-blue-700 transition-colors shadow-sm">
            Simpan
      ...
      <section class="bg-surface border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col mb-8">
        <div class="p-5 border-b border-border">
          <h2 class="text-[15px] font-bold text-t1">Pengaturan Fitur</h2>
        </div>
        <div class="flex flex-col divide-y divide-border/60">
          <div class="p-5 flex items-center justify-between gap-4">
            <div>
              <div class="text-[13.5px] font-bold text-t1">Pendaftaran warga baru</div>
              <div class="text-[12px] text-t2 mt-0.5">Izinkan warga daftar mandiri tanpa persetujuan admin</div>
            </div>
            <div class="w-11 h-6 bg-border rounded-full relative cursor-pointer shadow-inner shrink-0 transition-colors">
              <div class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full shadow-sm"></div>
            </div>
          </div>
          <div class="p-5 flex items-center justify-between gap-4">
            <div>
              <div class="text-[13.5px] font-bold text-t1">Notifikasi WhatsApp otomatis</div>
              <div class="text-[12px] text-t2 mt-0.5">Kirim WA otomatis saat status surat berubah atau pengumuman baru</div>
            </div>
            <div class="w-11 h-6 bg-blue-600 rounded-full relative cursor-pointer shadow-inner shrink-0 transition-colors">
