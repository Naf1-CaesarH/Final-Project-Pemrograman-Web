...
<head>
  ...
  <title>Data Warga — Portal Desa Digital</title>
...
<body class="bg-base font-sans text-t1 flex min-h-screen">
  ...
  <aside class="sidebar-fixed w-64 bg-slate-900 text-slate-300 flex flex-col justify-between sticky top-0 h-screen shadow-xl z-20">
    <div>
      ...
      <nav class="py-4 flex-1 overflow-y-auto scrollbar-hide">
        ...
        <a class="sb-item active !bg-blue-600/10 !text-blue-400 !border-r-4 !border-blue-500 px-5 py-2.5 flex items-center gap-3 cursor-pointer transition-colors" onclick="go('warga')">
          ...
          <span class="font-semibold text-[13px]">Data Warga</span>
        </a>
  ...
  <div class="flex-1 flex flex-col min-h-screen relative overflow-hidden">
    <header class="h-[60px] bg-surface border-b border-border flex items-center justify-between px-6 sticky top-0 z-10">
      <div class="flex items-center gap-2 text-[14px]">
        ...
        <span class="text-t1 font-bold">Data Warga</span>
      </div>
      <div class="flex items-center gap-4">
        ...
        <button type="button" class="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[13px] font-bold transition-colors shadow-sm" onclick="document.getElementById('modal-tambah-warga').classList.remove('hidden')">
    ...
    <main class="p-6 flex flex-col gap-6 flex-1 max-w-7xl mx-auto w-full">
      <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-surface border border-border rounded-2xl p-4 shadow-sm flex flex-col justify-between">
          ...
          <div class="text-[11.5px] text-t3 font-medium mt-1">Seluruh RT/RW</div>
        </div>
        <div class="bg-surface border border-border rounded-2xl p-4 shadow-sm flex flex-col justify-between">
          <div class="text-[12px] font-semibold text-t3 uppercase tracking-wider mb-2">Warga Aktif</div>
          <div class="text-[26px] font-bold text-green-600">304</div>
          <div class="text-[11.5px] text-t3 font-medium mt-1">Akun aktif</div>
        </div>
        <div class="bg-surface border border-border rounded-2xl p-4 shadow-sm flex flex-col justify-between">
          <div class="text-[12px] font-semibold text-t3 uppercase tracking-wider mb-2">Warga Non-aktif</div>
          <div class="text-[26px] font-bold text-t3">8</div>
          <div class="text-[11.5px] text-t3 font-medium mt-1">Akun dinonaktifkan</div>
        </div>
        <div class="bg-surface border border-border rounded-2xl p-4 shadow-sm flex flex-col justify-between">
          <div class="text-[12px] font-semibold text-t3 uppercase tracking-wider mb-2">Daftar Bulan Ini</div>
          <div class="text-[26px] font-bold text-t1">5</div>
          <div class="text-[11.5px] text-green-600 font-bold mt-1">↑ Warga baru</div>
        </div>
      </section>
      ...
      <div class="flex items-center justify-between gap-4 flex-wrap">
        <div class="relative w-full md:w-80">
          ...
          <input type="text" id="searchInput" onkeyup="filterWarga()" class="w-full bg-surface border border-border rounded-xl pl-10 pr-4 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" placeholder="Cari nama, NIK, atau nomor WhatsApp...">
        </div>
        <div class="flex flex-wrap bg-surface border border-border rounded-lg p-1">
          <button type="button" onclick="setFilterWarga('semua', this)" class="filter-btn px-4 py-1.5 text-[12px] font-bold bg-base text-t1 rounded-md shadow-sm">Semua RT/RW</button>
          <button type="button" onclick="setFilterWarga('rt 001', this)" class="filter-btn px-4 py-1.5 text-[12px] font-medium text-t2 hover:text-t1 rounded-md transition-colors">RT 001</button>
          <button type="button" onclick="setFilterWarga('rt 002', this)" class="filter-btn px-4 py-1.5 text-[12px] font-medium text-t2 hover:text-t1 rounded-md transition-colors">RT 002</button>
          <button type="button" onclick="setFilterWarga('rt 003', this)" class="filter-btn px-4 py-1.5 text-[12px] font-medium text-t2 hover:text-t1 rounded-md transition-colors">RT 003</button>
          <button type="button" onclick="setFilterWarga('rw 002', this)" class="filter-btn px-4 py-1.5 text-[12px] font-medium text-t2 hover:text-t1 rounded-md transition-colors">RW 002</button>
        </div>
      </div>
          ...
          <table class="w-full text-left text-[13px]">
            <thead class="bg-base border-b border-border text-t3 text-[10px] font-bold uppercase tracking-wider">
              <tr>
                ...
                <th class="px-5 py-3 text-center">Bergabung</th>
                <th class="px-5 py-3 text-center">Surat</th>
                <th class="px-5 py-3">Status</th>
                <th class="px-5 py-3 text-right">Aksi</th>
              </tr>
            </thead>
            <tbody id="tableBody" class="divide-y divide-border/60">
              <tr class="hover:bg-base/50 transition-colors">
                <td class="px-5 py-3">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-blue-100 text-blue-700 font-bold text-[12px] flex items-center justify-center flex-shrink-0">BS</div>
                    <div>
                      <div class="font-bold text-t1">Budi Santoso</div>
                      <div class="text-[11px] text-t3">budi@portaldesa.id</div>
                    </div>
                  </div>
                </td>
                <td class="px-5 py-3 font-mono text-t2">3201...0004</td>
                <td class="px-5 py-3 font-mono text-t2">0812-3333-3333</td>
                <td class="px-5 py-3 text-t2">RT 001/RW 001</td>
                <td class="px-5 py-3 text-center font-mono text-t2">Jan 2025</td>
                <td class="px-5 py-3 text-center font-mono font-bold text-t1">3</td>
                <td class="px-5 py-3">
                  <span class="bg-green-100 text-green-700 px-2.5 py-0.5 rounded-full text-[10px] font-bold">Aktif</span>
                </td>
                <td class="px-5 py-3">
                  <div class="flex gap-2 justify-end">
                    <button type="button" class="px-3 py-1 rounded border border-border bg-white text-t2 text-[11px] font-bold hover:bg-base transition-colors">Edit</button>
                    <button type="button" class="px-3 py-1 rounded border border-red-200 bg-red-50 text-red-600 text-[11px] font-bold hover:bg-red-100 transition-colors">Nonaktif</button>
                  </div>
                </td>
              </tr>
              <tr class="hover:bg-base/50 transition-colors">
                <td class="px-5 py-3">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-amber-100 text-amber-700 font-bold text-[12px] flex items-center justify-center flex-shrink-0">SR</div>
                    <div>
                      <div class="font-bold text-t1">Siti Rahayu</div>
                      <div class="text-[11px] text-t3">siti@portaldesa.id</div>
                    </div>
                  </div>
                </td>
                <td class="px-5 py-3 font-mono text-t2">3201...0005</td>
                <td class="px-5 py-3 font-mono text-t2">0812-4444-4444</td>
                <td class="px-5 py-3 text-t2">RT 001/RW 001</td>
                <td class="px-5 py-3 text-center font-mono text-t2">Jan 2025</td>
                <td class="px-5 py-3 text-center font-mono font-bold text-t1">1</td>
                <td class="px-5 py-3">
                  <span class="bg-green-100 text-green-700 px-2.5 py-0.5 rounded-full text-[10px] font-bold">Aktif</span>
                </td>
                <td class="px-5 py-3">
              ...
              <tr class="hover:bg-base/50 transition-colors">
                <td class="px-5 py-3">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-purple-100 text-purple-700 font-bold text-[12px] flex items-center justify-center flex-shrink-0">AP</div>
                    <div>
                      <div class="font-bold text-t1">Andi Pratama</div>
                      <div class="text-[11px] text-t3">andi@portaldesa.id</div>
                    </div>
                  </div>
                </td>
                <td class="px-5 py-3 font-mono text-t2">3201...0006</td>
                <td class="px-5 py-3 font-mono text-t2">0812-5555-5555</td>
                <td class="px-5 py-3 text-t2">RT 002/RW 001</td>
                <td class="px-5 py-3 text-center font-mono text-t2">Feb 2025</td>
                <td class="px-5 py-3 text-center font-mono font-bold text-t1">2</td>
                <td class="px-5 py-3">
                  <span class="bg-green-100 text-green-700 px-2.5 py-0.5 rounded-full text-[10px] font-bold">Aktif</span>
                </td>
                <td class="px-5 py-3">
              ...
              <tr class="hover:bg-base/50 transition-colors opacity-70">
                <td class="px-5 py-3">
                  <div class="flex items-center gap-3">
                    <div class="w-9 h-9 rounded-full bg-slate-200 text-slate-500 font-bold text-[12px] flex items-center justify-center flex-shrink-0">YP</div>
                    <div>
                      <div class="font-bold text-t3">Yanto Prabowo</div>
                      <div class="text-[11px] text-t3">yanto@portaldesa.id</div>
                    </div>
                  </div>
                </td>
                <td class="px-5 py-3 font-mono text-t3">3201...0009</td>
                <td class="px-5 py-3 font-mono text-t3">0812-7777-7777</td>
                <td class="px-5 py-3 text-t3">RT 003/RW 001</td>
                <td class="px-5 py-3 text-center font-mono text-t3">Mar 2025</td>
                <td class="px-5 py-3 text-center font-mono font-bold text-t3">0</td>
                <td class="px-5 py-3">
                  <span class="bg-slate-100 text-slate-600 px-2.5 py-0.5 rounded-full text-[10px] font-bold">Non-aktif</span>
                </td>
                <td class="px-5 py-3">
                  <div class="flex gap-2 justify-end">
                    <button type="button" class="px-3 py-1 rounded border border-green-200 bg-green-50 text-green-600 text-[11px] font-bold hover:bg-green-100 transition-colors">Aktifkan</button>
                  </div>
                </td>
              </tr>
            </tbody>
  ...
  <!-- Modal Tambah Warga -->
  <div id="modal-tambah-warga" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4 transition-opacity" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-surface w-full max-w-lg rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
      <div class="px-6 py-4 border-b border-border flex items-center justify-between">
        <h3 class="text-[16px] font-bold text-t1">Tambah Warga Baru</h3>
        <button type="button" aria-label="Tutup" class="text-t3 hover:text-red-500 transition-colors" onclick="document.getElementById('modal-tambah-warga').classList.add('hidden')">
      ...
      <div class="p-6 overflow-y-auto flex flex-col gap-4">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-[12px] font-semibold text-t1 mb-1.5">Nama lengkap</label>
            <input type="text" class="w-full bg-base border border-border rounded-xl px-3 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Nama sesuai KTP">
          </div>
          <div>
            <label class="block text-[12px] font-semibold text-t1 mb-1.5">NIK</label>
            <input type="text" maxlength="16" class="w-full bg-base border border-border rounded-xl px-3 py-2.5 text-[13px] font-mono text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="16 digit NIK">
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-[12px] font-semibold text-t1 mb-1.5">Nomor WhatsApp</label>
            <input type="tel" class="w-full bg-base border border-border rounded-xl px-3 py-2.5 text-[13px] font-mono text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="08xxxxxxxxxx">
          </div>
          <div>
            <label class="block text-[12px] font-semibold text-t1 mb-1.5">Email</label>
            <input type="email" class="w-full bg-base border border-border rounded-xl px-3 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="email@portaldesa.id">
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-[12px] font-semibold text-t1 mb-1.5">RT</label>
            <select class="w-full bg-base border border-border rounded-xl px-3 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 appearance-none cursor-pointer">
              <option>RT 001</option><option>RT 002</option><option>RT 003</option>
            </select>
          </div>
          <div>
            <label class="block text-[12px] font-semibold text-t1 mb-1.5">RW</label>
            <select class="w-full bg-base border border-border rounded-xl px-3 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 appearance-none cursor-pointer">
              <option>RW 001</option><option>RW 002</option>
            </select>
          </div>
        </div>
        <div>
          <label class="block text-[12px] font-semibold text-t1 mb-1.5">Password awal</label>
          <input type="password" class="w-full bg-base border border-border rounded-xl px-3 py-2.5 text-[13px] text-t1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="Akan dikirim via WhatsApp">
        </div>
      </div>
      <div class="px-6 py-4 border-t border-border bg-slate-50 rounded-b-2xl flex justify-end gap-3">
        <button type="button" class="px-5 py-2.5 rounded-xl border border-border bg-white text-t1 text-[13px] font-bold hover:bg-base transition-colors" onclick="document.getElementById('modal-tambah-warga').classList.add('hidden')">Batal</button>
        <button type="button" class="px-5 py-2.5 rounded-xl bg-blue-600 text-white text-[13px] font-bold hover:bg-blue-700 transition-colors shadow-sm" onclick="document.getElementById('modal-tambah-warga').classList.add('hidden')">Simpan &amp; Kirim Akun</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    function setFilterWarga(filter, btn) {
    ...
    function filterWarga() {
      ...
      rows.forEach(row => {
        ...
        const rtCell = row.children[3] ? row.children[3].innerText.toLowerCase() : "";
        
        const matchSearch = text.includes(search);
        let matchFilter = true;
        
        if (currentFilter !== 'semua') {
          matchFilter = rtCell.includes(currentFilter);
        }
