...
<head>
  ...
  <title>Data Warga RT — Panel RT</title>
...
<body class="bg-base font-sans text-t1 flex min-h-screen">
  ...
  <aside class="sidebar-fixed w-64 bg-slate-900 text-slate-300 flex flex-col justify-between sticky top-0 h-screen shadow-xl z-20">
    <div>
      ...
      <nav class="py-4 flex-1 overflow-y-auto scrollbar-hide">
        ...
        <a class="sb-item active !bg-indigo-600/10 !text-indigo-400 !border-r-4 !border-indigo-500 px-5 py-2.5 flex items-center gap-3 cursor-pointer transition-colors" onclick="go('warga-data')">
          <svg width="15" height="15" viewBox="0 0 16 16" fill="none"><path d="M4 14V3h8v11" stroke="currentColor" stroke-width="1.5"/></svg>
          <span class="font-semibold text-[13px]">Data Warga RT</span>
        </a>
      </nav>
    </div>
  ...
  <div class="flex-1 flex flex-col min-h-screen relative overflow-hidden">
    ...
    <header class="h-[60px] bg-surface border-b border-border flex items-center px-6 gap-2 sticky top-0 z-10 text-[14px]">
      ...
      <strong class="text-t1 font-bold">Buku Data Warga</strong>
    </header>
    ...
    <main class="p-6 flex flex-col gap-6 flex-1 max-w-6xl w-full mx-auto">
      ...
      <div class="flex flex-col sm:flex-row items-center gap-4 justify-between bg-surface p-4 rounded-2xl border border-border shadow-sm">
        <div class="relative flex-1 w-full sm:max-w-md">
          ...
          <input id="searchInput" onkeyup="filterWarga()" class="w-full h-11 pl-11 pr-4 rounded-xl border border-border text-[13px] bg-base focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 shadow-sm transition-all" type="text" placeholder="Cari nama warga, NIK, No. KK..."/>
        </div>
        
        <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <div class="flex flex-wrap bg-base border border-border rounded-xl p-1">
            <button type="button" class="px-4 py-2 text-[12.5px] font-bold bg-indigo-50 text-indigo-700 rounded-lg shadow-sm border border-indigo-100 transition-colors" onclick="filterWarga('semua', this)">Semua (142)</button>
            <button type="button" class="px-4 py-2 text-[12.5px] font-medium text-t2 hover:text-t1 rounded-lg transition-colors" onclick="filterWarga('kepala keluarga', this)">Kepala Keluarga</button>
            <button type="button" class="px-4 py-2 text-[12.5px] font-medium text-t2 hover:text-t1 rounded-lg transition-colors whitespace-nowrap" onclick="filterWarga('kontrak/kos', this)">Warga Kos/Kontrak</button>
          </div>
          <button type="button" class="flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-bold transition-all shadow-md shadow-indigo-500/20" onclick="go('#')">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M4 8h8M8 4v8" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> Tambah
          </button>
        </div>
      </div>
          ...
          <table class="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr class="bg-base border-b border-border/80 text-[11px] text-t3 uppercase tracking-wider font-bold">
                <th class="py-4 px-6 rounded-tl-lg">Nama Lengkap</th>
                <th class="py-4 px-6">NIK & No. KK</th>
                <th class="py-4 px-6">Status Dlm Keluarga</th>
                <th class="py-4 px-6">No. WhatsApp</th>
                <th class="py-4 px-6">Status Rumah</th>
            ...
            <tbody class="text-[13.5px]">
              ...
              <tr class="border-b border-border/60 hover:bg-indigo-50/30 transition-colors group">
                ...
                <td class="py-4 px-6">
                  <div class="font-mono font-medium text-t2 group-hover:text-t1 transition-colors">3201010101010004</div>
                  <div class="text-[11.5px] text-t3 mt-0.5">KK: <span class="font-mono">32010122222222</span></div>
                </td>
                <td class="py-4 px-6 text-t1 font-bold">Kepala Keluarga</td>
                <td class="py-4 px-6 text-t2 font-medium">0812-3333-3333</td>
                <td class="py-4 px-6">
                  <span class="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[11.5px] font-bold px-2.5 py-1 rounded-lg">Milik Sendiri</span>
                </td>
                <td class="py-4 px-6 text-right">
                  <button type="button" class="text-indigo-600 hover:text-indigo-800 font-bold text-[13px] hover:underline px-2 py-1" onclick="go('#')">Detail</button>
                </td>
              </tr>
              ...
              <tr class="border-b border-border/60 hover:bg-indigo-50/30 transition-colors group">
                ...
                <td class="py-4 px-6">
                  <div class="font-mono font-medium text-t2 group-hover:text-t1 transition-colors">3201010101010005</div>
                  <div class="text-[11.5px] text-t3 mt-0.5">KK: <span class="font-mono">32010122222222</span></div>
                </td>
                <td class="py-4 px-6 text-t1 font-medium">Istri</td>
                <td class="py-4 px-6 text-t2 font-medium">0812-4444-4444</td>
                <td class="py-4 px-6">
                  <span class="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[11.5px] font-bold px-2.5 py-1 rounded-lg">Milik Sendiri</span>
                </td>
                <td class="py-4 px-6 text-right">
              ...
              <tr class="hover:bg-indigo-50/30 transition-colors group">
                ...
                <td class="py-4 px-6">
                  <div class="font-mono font-medium text-t2 group-hover:text-t1 transition-colors">3201010101010008</div>
                  <div class="text-[11.5px] text-t3 mt-0.5">KK: <span class="font-mono">32010155555555</span></div>
                </td>
                <td class="py-4 px-6 text-t1 font-bold">Kepala Keluarga</td>
                <td class="py-4 px-6 text-t2 font-medium">0812-9999-9999</td>
                <td class="py-4 px-6">
                  <span class="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 border border-blue-200 text-[11.5px] font-bold px-2.5 py-1 rounded-lg">Kontrak/Kos</span>
                </td>
                <td class="py-4 px-6 text-right">
                  <button type="button" class="text-indigo-600 hover:text-indigo-800 font-bold text-[13px] hover:underline px-2 py-1" onclick="go('#')">Detail</button>
                </td>
  ...
  <script>
    ...
    let currentWargaFilter = 'semua';

    function filterWarga(type, btnElement) {
      if (type) currentWargaFilter = type;
      ...
      rows.forEach(row => {
        ...
        const statusKeluarga = row.children[2] ? row.children[2].innerText.toLowerCase() : "";
        const statusRumah = row.children[4] ? row.children[4].innerText.toLowerCase() : "";
        
        let matchFilter = true;
        if (currentWargaFilter === 'kepala keluarga') {
          matchFilter = statusKeluarga.includes('kepala keluarga');
        } else if (currentWargaFilter === 'kontrak/kos') {
          matchFilter = statusRumah.includes('kontrak') || statusRumah.includes('kos');
        }
        
        if (text.includes(search) && matchFilter) {
          row.style.display = '';
        } else {
